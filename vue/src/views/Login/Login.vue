...
export default {
  ...
  methods:{
    ...
    login(){
      ...
      let oldversion
      let oldid
      oldid = thus.$store.state.userId
      oldversion = thus.$store.state.indexdbVersion
      ...
      axios.get('/login?name=' + this.name + '&pwd=' + this.pwd)
          .then(function (response) {
            ...
            if(response.data.code == null){
              ...
              if(thus.$store.state.userId!==oldid){
                localStorage.setItem('indexdbVersion',Number(oldversion)+1)
                thus.$store.dispatch('setIndexdbVersion',Number(oldversion)+1)
              }
